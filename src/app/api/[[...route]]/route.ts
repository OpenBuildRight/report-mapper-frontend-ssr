...
app.openapi(listObservationsRoute, async (c) => {
...
app.openapi(getObservationRoute, async (c) => {
...
app.openapi(updateObservationRoute, async (c) => {
  const context = await requireAuth(c.req.raw as Request)
  const { id } = c.req.valid('param')
  const body = c.req.valid('json')
...
app.openapi(deleteObservationRoute, async (c) => {
  const context = await requireAuth(c.req.raw as Request)
  const { id } = c.req.valid('param')
...
app.openapi(publishObservationRoute, async (c) => {
