...
export const observationFiltersSchema = z.object({
  ...
  submitted: z.enum(['true', 'false']).transform(val => val === 'true').optional(),
  revisionId: z.string().transform(val => parseInt(val, 10)).optional(),
  latestOnly: z.enum(['true', 'false']).transform(val => val === 'true').optional(),
  ...
  minLat: z.string().transform(val => parseFloat(val)).optional(),
  maxLat: z.string().transform(val => parseFloat(val)).optional(),
  minLng: z.string().transform(val => parseFloat(val)).optional(),
  maxLng: z.string().transform(val => parseFloat(val)).optional(),
...
export type CreateObservationRevisionInput = z.infer<typeof createObservationRevisionSchema>
export type UpdateObservationRevisionInput = z.infer<typeof updateObservationRevisionSchema>
export type RevisionActionInput = z.infer<typeof revisionActionSchema>
export type CreateImageRevisionInput = z.infer<typeof createImageRevisionSchema>
export type UpdateImageRevisionInput = z.infer<typeof updateImageRevisionSchema>
