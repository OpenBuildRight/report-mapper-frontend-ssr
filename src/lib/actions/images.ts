 ...
 */
export class ImageController extends RevisionController<
  ImageFields,
...
> {
  ...
  async getRevisionWithUrl(
    ...
    const revision = await this.getRevision(itemId, revisionId);
    return {
      ...revision,
  ...
  async getImagesByReferences(
    ...
    for (const ref of imageRefs) {
      try {
        ...
        console.error(
          `Failed to fetch image ${ref.id} revision ${ref.revisionId}:`,
          error,
        );
  ...
  getImageUrls(imageRefs: ImageReference[]): string[] {
    return imageRefs.map(
      (ref) => `/api/images/${ref.id}/file?revisionId=${ref.revisionId}`,
    );
  }
}
