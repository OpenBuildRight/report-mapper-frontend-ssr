...
export async function initializeDatabase() {
  try {
    ...
    await db
      .collection(COLLECTIONS.OBSERVATION_REVISIONS)
      .createIndex({ observation_id: 1, revision_id: 1 }, { unique: true });

    // Common query patterns
    await db
      .collection(COLLECTIONS.OBSERVATION_REVISIONS)
      .createIndex({ observation_id: 1, published: 1 });
    await db
      .collection(COLLECTIONS.OBSERVATION_REVISIONS)
      .createIndex({ owner: 1, published: 1 });
    await db
      .collection(COLLECTIONS.OBSERVATION_REVISIONS)
      .createIndex({ owner: 1, submitted: 1 });
    await db
      .collection(COLLECTIONS.OBSERVATION_REVISIONS)
      .createIndex({ published: 1 });
    await db
      .collection(COLLECTIONS.OBSERVATION_REVISIONS)
      .createIndex({ submitted: 1, published: 1 });
    ...
    await db
      .collection(COLLECTIONS.OBSERVATION_REVISIONS)
      .createIndex({ published: 1, revision_created_at: -1 });
    await db
      .collection(COLLECTIONS.OBSERVATION_REVISIONS)
      .createIndex({ submitted: 1, published: 1, revision_created_at: -1 });
    await db
      .collection(COLLECTIONS.OBSERVATION_REVISIONS)
      .createIndex({ owner: 1, published: 1, revision_created_at: -1 });

    // Image revisions indexes
    await db
      .collection(COLLECTIONS.IMAGE_REVISIONS)
      .createIndex({ id: 1, revision_id: 1 }, { unique: true });
    await db
      .collection(COLLECTIONS.IMAGE_REVISIONS)
      .createIndex({ id: 1, published: 1 });
    await db.collection(COLLECTIONS.IMAGE_REVISIONS).createIndex({ owner: 1 });
    await db
      .collection(COLLECTIONS.IMAGE_REVISIONS)
      .createIndex({ image_metadata_location: "2dsphere" }); // Geospatial index
